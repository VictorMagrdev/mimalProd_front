...
const { data: dataDep } = await useAsyncQuery<{ depreciaciones: Depreciacion[] }>(
  GetDepreciaciones
)
depreciaciones.value = dataDep.value?.depreciaciones.filter(d => d.maquinaId === maquinaId) || []

const depreciacionProyectada = computed<PuntoGrafico[]>(() => {
  if (!maquina.value) return []
  const { costoCompra, valorRescate, vidaUtilAnios, fechaCompra } = maquina.value
...
const graphData = computed<PuntoGrafico[]>(() => {
  const reales: PuntoGrafico[] = depreciaciones.value.map(d => {
    const año = new Date(d.periodo).getFullYear() - new Date(maquina.value!.fechaCompra).getFullYear()
    return {
      año,
      fecha: d.periodo,
