...
// Requests
const { data: dataIncidencias, pending: pendingIncidencias } = await useAsyncQuery<{ incidencias: Omit<Incidencia, 'archivos'>[] }>(GET_TODAS_INCIDENCIAS)
const { data: dataArchivos, pending: pendingArchivos } = await useAsyncQuery<{ incidenciasArchivo: Archivo[] }>(GET_TODOS_ARCHIVOS)

// Filtramos la incidencia y sus archivos
const incidencia = computed<Incidencia | null>(() => {
  const inc = dataIncidencias.value?.incidencias.find(i => i.id === incidenciaId)
  if (!inc) return null

  return {
    ...inc,
    archivos: dataArchivos.value?.incidenciasArchivo.filter(a => a.incidenciaId === incidenciaId && a.id) || []
  }
})

const pending = computed(() => pendingIncidencias.value || pendingArchivos.value)
    ...
    <UCard>
      ...
      <div v-else-if="incidencia" class="space-y-6">
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <UDescription label="Tipo" :value="incidencia.tipoIncidenciaId || 'N/A'" />
          <UDescription label="Máquina" :value="incidencia.maquinaId || 'N/A'" />
          <UDescription
            label="Creado"
        ...
        <div v-if="incidencia?.archivos?.length">
          ...
          >
            <div
              v-for="archivo in incidencia.archivos.filter(a => a?.tipo === 'FOTO') || []"
              :key="archivo.id"
          ...
          >
            <div
              v-for="archivo in incidencia.archivos.filter(a => a?.tipo === 'AUDIO') || []"
              :key="archivo.id"
            ...
            >
              ...
              <div class="flex justify-between w-full items-center">
                <span class="text-sm text-gray-600 truncate">{{ archivo.nombreOriginal }}</span>
                <UButton
                  icon="i-heroicons-arrow-down-tray"
                  color="neutral"
                  variant="outline"
