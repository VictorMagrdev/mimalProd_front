...
const state = reactive({ codigo: '', nombre: '', descripcion: '' });

const { result, loading: queryLoading } = useQuery(GetUnidadMedidaTipoById, { id: computed(() => props.tipoId) }, { enabled: computed(() => !!props.tipoId) });
...
async function onSubmit() {
  ...
  try {
    ...
    toast.add({ title: 'Éxito', description: 'Tipo de unidad de medida actualizado.' });
    emit('updated');
    closeModal();
  } catch (error: unknown) {
...
<template>
  <UModal :model-value="props.isOpen" @update:model-value="closeModal">
    <UCard>
      <template #header><h2>Actualizar Tipo de Unidad de Medida</h2></template>
