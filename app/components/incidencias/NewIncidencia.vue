...
const estacionesProduccion = computed(() => result.value?.estacionesProduccion ?? []);
...
const onSubmit = async () => {
  try {
    ...
    formData.append("archivos", new File([audioBlob.value], "grabacion.webm", { type: "audio/webm" }));
...
<template>
  <UModal v-model:open="open" title="Crear Incidencia" description="Complete el formulario para registrar una nueva incidencia." size="lg">
      ...
      >
        <div class="grid grid-cols-2 gap-4">
          <UFormField label="Código" name="codigo">
            <UInput v-model="state.codigo" placeholder="INC-001" class="w-full"/>
          </UFormField>

          <UFormField label="Título" name="titulo">
            <UInput v-model="state.titulo" placeholder="Título de la incidencia" class="w-full" />
          </UFormField>
        </div>

        <UFormField label="Descripción" name="descripcion">
          <UTextarea v-model="state.descripcion" placeholder="Descripción detallada..." class="w-full" />
        </UFormField>

          <UFormField label="Tipo de Incidencia" name="tipoIncidenciaId">
            <USelect v-model="state.tipoIncidenciaId" :items="tiposIncidencia" placeholder="Seleccione tipo" class="w-full" />
          </UFormField>

          <UFormField label="Estado" name="estadoId">
            <USelect v-model="state.estadoId" :items="estadosIncidencia" placeholder="Seleccione estado" class="w-full" />
          </UFormField>

          <UFormField label="Máquina" name="maquinaId">
            <USelect v-model="state.maquinaId" :items="maquinas" placeholder="Seleccione máquina" class="w-full"/>
          </UFormField>
          <UFormField label="Orden Producción" name="ordenId">
            <USelect v-model="state.ordenId" :items="ordenesProduccion" placeholder="Seleccione orden" class="w-full"/>
          </UFormField>
          <UFormField label="Estación" name="estacionId">
            <USelect v-model="state.estacionId" :items="estacionesProduccion" placeholder="Seleccione estación" class="w-full"/>
          </UFormField>

          <UFormField label="Fecha de Cierre" name="fechaCierre">
            <UInput v-model="state.fechaCierre" type="datetime-local" class="w-full" />
          </UFormField>
          <UFormField label="Tiempo de Parada (horas)" name="tiempoParada">
            <UInput v-model="state.tiempoParada" type="number" step="0.1" placeholder="0.0" class="w-full"/>
          </UFormField>
    ...
    <template #footer="{ close }">
      <UButton label="Cancelar" color="neutral" variant="outline" @click="() => { close(); resetForm(); }" />
      <UButton
