...
const state = reactive({ numeroLote: '', idProducto: undefined, fabricadoEn: '', venceEn: '' });

const { result, loading: queryLoading } = useQuery(GetLoteProduccionById, { id: computed(() => props.loteId) }, { enabled: computed(() => !!props.loteId) });

watch(result, (newVal) => {
  if (newVal?.loteProduccion) {
    const { numeroLote, fabricadoEn, venceEn, producto } = newVal.loteProduccion;
    Object.assign(state, { numeroLote, fabricadoEn, venceEn, idProducto: producto?.id });
  }
});
