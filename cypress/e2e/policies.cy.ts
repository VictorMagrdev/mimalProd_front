describe('Funcionalidad de Gestión de Políticas', () => {
  ...
  context('Dado que el usuario está en la página de gestión de políticas', () => {
    ...
    context('Cuando el usuario crea una nueva política', () => {
      it('Entonces se debe mostrar un mensaje de éxito', () => {
        ...
        cy.get('textarea[data-cy=policy-description]').type('Permite al usuario ver el dashboard principal');
        cy.get('[data-cy=create-policy-button]').click();
        cy.get('.success-message').should('contain', 'Política creada correctamente');
      });
    });
  });
});